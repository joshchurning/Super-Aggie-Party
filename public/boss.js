...
var bossText = [
	...
	"threw a sucker punch while you weren't looking! You get winded and feel it instantly.",
	"let out a barrage of punches! It's pretty effective."
];

function bossTextDialogue(damage) {//boss damage is between 0 to 10
	if(damage == 0) return bossText[0];
	else if(damage > 0 && damage <= 3) return bossText[1];
	else if(damage > 3 && damage <= 7) return bossText[2];
	else if(damage > 7) return bossText[3];
}
...
function runBoss() {
    ...
    function preload(){
		...
		game.load.image('boss', 'boss/ukBoss.png');
    }
    ...
    function create(){
		...
		bDialogueText = game.add.text(605, 700, "A wild Wildcat draws near!", style1);
		bDialogueText.alpha = 1;
		//var pTextTween = game.add.tween(pDialogueText).to({alpha: 0}, 2750, Phaser.Easing.Linear.None, true, 0, 0, 0);
		//var bTextTween = game.add.tween(bDialogueText).to({alpha: 0}, 2750, Phaser.Easing.Linear.None, true, 0, 0, 0);
    ...
    function attackBoss(x){
        ...
        if(bossHealth > 0){
            ...
            if(bossHealth <= 0){ // Player defeated the boss
				...
				textFeedback = game.add.text(game.world.centerX, game.height - 45, "You beat the boss! You will receive $50", style);
				...
				setTimeout(function(){document.getElementById("start").style.visibility = "visible";
					...
					App[App.myRole].boss1 = true;                
					UpdatePlayer(50);
					socket.emit("updateBoard", App);
					//alert("You win! Your reward is: 25"); //also if this is true it should go to next screen so we have to add it. add some reward
					game.destroy();}
					,3250);
            }
        }
    ...
    function checkStatus(){
        if(playerHealth <= 0){ //Boss defeated the player
			var style = {font: "42px Arial", fill: "#ffffff"};
            ...
            if((pencilRemaining + penRemaining + calculatorRemaining + bookRemaining + laptopRemaining) === 0) {
    ...
    function bossAttacks(){
        ...
        var randDamager = Math.floor(Math.random() * (10 - 0 + 1)) + 0; //format is math.random() * (max - min + 1) + min....just in case we want to switch numbers for different bosses.
        var tempDialogue = bossTextDialogue(randDamager);
