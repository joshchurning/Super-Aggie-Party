...
function bossTextDialogue2(damage) {//boss damage is between 0 to 20
	if(damage == 0) return bossText2[0];
	else if(damage > 0 && damage <= 7) return bossText2[1];
	else if(damage > 7 && damage <= 14) return bossText2[2];
	else if(damage > 14) return bossText2[3];
}
...
function runBoss2() {
    ...
    var bossHealth = 50; //this would change every boss stage to a bigger number
	var bossName = "An Offsides Tiger";
    ...
    function preload(){
		...
		game.load.image('boss', 'boss/lsuBoss.png');
    }
    ...
    function create(){
        ...
        bHealthText = game.add.text(vsText.x + (vsText.x/2), pHealthText.y, "Health:  " + bossHealth + "/50", style1);
		...
		bDialogueText = game.add.text(605, 700, "An offsides Tiger draws near!", style1);
    ...
    function attackBoss(x){
        ...
        if(bossHealth > 0){
			...
			bossHealth -= x;
            bHealthText.text =  "Health:  " + bossHealth + "/50" 
            if(bossHealth <= 0){ // Player defeated the boss
				var style = {font: "42px Arial", fill: "#ffffff"};
				textFeedback = game.add.text(game.world.centerX, game.height - 45, "You beat the boss! You will receive $100", style);
    ...
    function bossAttacks(){
        ...
        var randDamager = Math.floor(Math.random() * (20 - 0 + 1)) + 0; //format is math.random() * (max - min + 1) + min....just in case we want to switch numbers for different bosses.
        var tempDialogue = bossTextDialogue2(randDamager);
