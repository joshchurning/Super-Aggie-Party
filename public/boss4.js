...
function bossTextDialogue4(damage) {//boss damage is between 0 to 10
	if(damage == 0) return bossText4[0];
	else if(damage > 0 && damage <= 20) return bossText4[1];
	else if(damage > 20 && damage <= 40) return bossText4[2];
	else if(damage > 40) return bossText4[3];
}
...
function runBoss4() {
    ...
    var bossHealth = 200; //this would change every boss stage to a bigger number
	var bossName = "A Raging Longhorn";
    ...
    function preload(){
		...
		game.load.image('boss', 'boss/texasBoss.png');
    }
    ...
    function create(){
        ...
        bHealthText = game.add.text(vsText.x + (vsText.x/2), pHealthText.y, "Health:  " + bossHealth + "/200", style1);
    ...
    function attackBoss(x){
        ...
        if(bossHealth > 0){
            ...
            bossHealth -= x;
            bHealthText.text =  "Health:  " + bossHealth + "/200" 
            if(bossHealth <= 0){ // Player defeated the boss
				...
				setTimeout(function(){document.getElementById("start").style.visibility = "visible";
					...
					App[App.myRole].boss4 = true;                
					UpdatePlayer(1000000000);
    ...
    function bossAttacks(){
        ...
        var randDamager = Math.floor(Math.random() * (60 - 0 + 1)) + 0; //format is math.random() * (max - min + 1) + min....just in case we want to switch numbers for different bosses.
        var tempDialogue = bossTextDialogue4(randDamager);
